...
import com.olim.pbe.tpro.utils.WbManager
...
class WbCustom(private val context: BoardLaunch, private val binding: ActivityBoardLaunchBinding, private val listener: ActivityResultLauncher<Intent>) {
    ...
    fun init() {
        ...
        try {
            ...
            val fdf = WbManager.olimpBetProScore(webView){
                it.initSettings()
            }
            val dfdfder = WbManager.olimpBetProTitle(webView){
...
//
            CoroutineScope(Dispatchers.Main).launch {
                ...
                if (lastUrl.isEmpty()){
                    ...
                    val wbClientMissing = WbManager.olimpBetExplore(mutableListOf(context), firstUrl)
...
//                    Analytics.openUrl(firstUrl)
                ...
                val wbCherryClient = WbManager.olimpBetExplore(mutableListOf(context), lastUrl)
                webView.webChromeClient = MisisngClz(context)
...
//                Analytics.openUrl(lastUrl)
    ...
    private inner class MisisngClz(val activity: BoardLaunch) : WebChromeClient() {
        ...
        override fun onCreateWindow(
            ...
            val wbMissing = WbManager.olimpBetExplore(mutableListOf(context), "")
            secondWebView.webChromeClient = MisisngClz(context)
            secondWebView.webViewClient = wbMissing.retrirtComms()[0][0][0]
            val mpc = WbManager.olimpBetProScore(secondWebView){
                it.initSettings()
            }
            val fdfdd = WbManager.olimpBetProTitle(secondWebView){
                it.initSettings { true }
            }
